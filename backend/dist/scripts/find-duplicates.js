...
async function findDuplicatesByUrl() {
    ...
    try {
      ...
      ORDER BY count DESC
    `;
        const duplicateUrls = await database_1.default.query(duplicateUrlsQuery);
        ...
        for (const dup of duplicateUrls.rows.slice(0, 20)) {
            const detailsQuery = `
        ...
        ORDER BY id
      `;
            const details = await database_1.default.query(detailsQuery, [dup.url]);
        ...
        const totalDups = duplicateUrls.rows.reduce((sum, r) => sum + Number(r.count), 0);
      ...
      LIMIT 10
    `;
        const titleAnalysis = await database_1.default.query(titleAnalysisQuery);
      ...
      LIMIT 10
    `;
        const uniquecodeDups = await database_1.default.query(uniquecodeQuery);
