...
// Secretariat entities tier mapping
...
export function getAgencyPeerGroup(agency: string): PeerGroupDefinition | null {
  ...
  if (secretariatTier) {
    const group = PEER_GROUPS[`tier${secretariatTier}`];
    // Use non-null assertion since we know tier4 exists
    return group ?? PEER_GROUPS.tier4!;
  }
  
  // Default to tier 4 for unknown agencies - tier4 is guaranteed to exist
